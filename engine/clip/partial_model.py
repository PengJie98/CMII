...
class PartialViT(nn.Module):
    def __init__(self, dtype, conv1=None,
...
def get_split_vit(dtype, model, layer_idx=0):
    ...
    if layer_idx == -1:
        # finetune all layers
        feature_extractor = PartialViT(dtype, mode='feature_extractor')
        partial_model = PartialViT(dtype, conv1=conv1,
        ...
        # finetune no layers
        feature_extractor = PartialViT(dtype, conv1=conv1,
                                       class_embedding=class_embedding,
                                       positional_embedding=positional_embedding,
                                       ln_pre=ln_pre,
                                       transformer_encoder=transformer,
                                       ...
                                       proj=proj,
                                       mode='feature_extractor')
        partial_model = PartialViT(dtype, mode='partial_model')
    else:
        # finetune some layers
        transformer_encoder = transformer.resblocks[:-layer_idx]
        partial_transformer = transformer.resblocks[-layer_idx:]
        feature_extractor = PartialViT(dtype, conv1=conv1,
                                       ...
                                       transformer_encoder=transformer_encoder,
                                       mode='feature_extractor')
        partial_model = PartialViT(dtype, transformer_encoder=partial_transformer,
                                   ln_post=ln_post,
                                   proj=proj,
                                   mode='partial_model')
    feature_extractor.eval()
...
class PartialResNet(nn.Module):
    def __init__(self, conv1=None,
        ...
        self.apply_stem = self.conv3 is not None
        if mode == 'partial_model':
            if self.conv1 is not None:
...
def get_image_encoder(clip_encoder, image_layer_idx, clip_model):
    ...
    elif clip_encoder == "ViT-B/16" or clip_encoder == "ViT-L/14":  # Jamison add here.
        assert type(clip_model.visual) == VisionTransformer
        return get_split_vit(clip_model.dtype, clip_model.visual, image_layer_idx)
    else:
