...
def prepare_dataset(path, name):
    ...
    with open(path + 'entity2id.txt', 'r') as f:
        for line in f:
            ...
            entities_to_id[instance[0]] = int(instance[1])

    relations_to_id = {}
    with open(path + 'relation2id.txt', 'r') as f:
        for line in f:
            instance = line.strip().split()
            relations_to_id[instance[0]] = int(instance[1])

    entities = entities_to_id.keys()
    relations = relations_to_id.keys()
    ...
    for f in files:
        file_path = os.path.join(path, f + '.txt')
        to_read = open(file_path, 'r')
        examples = []
        for line in to_read.readlines():
            res = line.strip().split(' ')
            lhs, rel, rhs = res[0], res[1], res[2]
            try:
...
if __name__ == "__main__":
    ...
    for d in datasets:
        ...
        try:
            prepare_dataset(
                '/home/dw/backup_pj/data/{}/Text/'.format(d),
                d
            )
        except OSError as e:
